...
from PCVisionTools import VisionTaskRunner_PC, HyLine_PC, HyLocator_PC, HyPatMat_PC, HyLinePatMat_PC

class VirtualVisionEngine(QThread):
    """
    ...
    def __init__(self, cmd_queue, res_queue, image_source, interval_ms=100):
        ...
        self.task_runner = VisionTaskRunner_PC()
    ...
    def _process_command(self, cmd_data):
        ...
        # [동적 프로비저닝 명령] (장치에 툴 심기)
            ...
            if tool_type == 1: # HyLine
                self.task_runner.add_tool(HyLine_PC(target_id, seq_id, (x,y,w,h)))
            elif tool_type == 2: # HyPatMat (템플릿 처리는 0x11 구현 후 추가 연결)
                self.task_runner.add_tool(HyPatMat_PC(target_id, seq_id, (x,y,w,h), []))
            elif tool_type == 3: # HyLocator (임시 더미 매핑)
                dummy_target = HyLine_PC(0, 0, (0,0,0,0)) 
                self.task_runner.add_tool(HyLocator_PC(target_id, seq_id, dummy_target, (x,y,w,h), (-15, 15)))
